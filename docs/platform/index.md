...
| **Risk-score model** | [risk-score-model-generic](https://github.com/CivicDataLab/risk-score-model-generic) | [Risk model](../datasources/data-model.md) | Generic, methodology-driven scoring model used to produce indicator values |
...
The [IDS-DRR repository](https://github.com/CivicDataLab/IDS-DRR) ties the core platform together via its [docker-compose.yml](https://github.com/CivicDataLab/IDS-DRR/blob/main/docker-compose.yml) file. **Frontend**, **Data Management**, and the **risk-score model** are git submodules of that repository; per-deployment branding/data packages are separate repositories to clone or create alongside, as needed.
...
4. **Build a frontend branding package.** A small TypeScript npm package supplies the logo, hero imagery, region list, locale messages, tile-layer URLs, etc. See [Frontend → Configuration](frontend.md#configuration).
